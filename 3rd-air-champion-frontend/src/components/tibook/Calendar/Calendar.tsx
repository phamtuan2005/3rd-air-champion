...
const Calendar = ({
  ...
  const usedRooms = useMemo(
    () =>
      rooms
        .filter(
          (r) => r.active && (!selectedRoomName || r.name === selectedRoomName),
        )
  ...
  const numRows =
    containerHeight > 0
      ? Math.max(Math.floor(containerHeight / minRowHeight), 1)
      : 5;
  const rowHeight =
  ...
  useEffect(() => {
    ...
    for (const month of months) {
      ...
      const pageStart: Date =
        overflowDate ?? new Date(month.getFullYear(), month.getMonth(), 1);
  ...
  const customTile = ({ date, pageMonth }: { date: Date; pageMonth: Date }) => {
    ...
    if (!isSameMonth(date, pageMonth))
      className.push("react-calendar__custom_tile_outside");

    return className;
  };

  const customTileContent = (date: Date) => {
    const day = monthMap.get(date.toISOString().split("T")[0]);
      ...
      day && selectedRoomName
        ? {
            ...
            bookings: day.bookings.filter(
              (b) => b.room?.name === selectedRoomName,
            ),
          }
        : day;
      ...
      <>
        {sortedUsedRooms.map((room) => {
          ...
          const amIsEnd = amBooking?.endDate
            ? isSameDay(
                date,
                addDays(toZonedTime(amBooking.endDate, timeZone), 1),
              )
            : false;
          ...
          const roomLabel = pmBooking?.room?.name ?? "";
          ...
          return (
            ...
            >
              ...
              {pmBooking ? (
                <div
                  className={`${pmColor} text-black font-bold flex items-center`}
                  style={{
                    position: "absolute",
                    top: "-1px",
                    bottom: "-1px",
            ...
            {inWindow && layout.cells.length > 0 && (
              ...
              >
                {layout.cells.map((date, cellIdx) => {
                  ...
                  const classes = customTile({ date, pageMonth: layout.month });
                  const content = customTileContent(date);
