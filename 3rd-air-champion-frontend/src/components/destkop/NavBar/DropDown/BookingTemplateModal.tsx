...
const BookingTemplateModal = ({ onClose }: BookingTemplateModalProps) => {
  const [draft, setDraft] = useState(
    () => localStorage.getItem(BOOKING_TEMPLATE_KEY) || DEFAULT_BOOKING_TEMPLATE,
  );
  const textareaRef = useRef<HTMLTextAreaElement>(null);

  const save = () => {
    localStorage.setItem(BOOKING_TEMPLATE_KEY, draft);
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg p-4 w-full max-w-lg shadow-lg flex flex-col gap-3">
        <div className="flex flex-col items-center gap-1">
          ...
          <h2 className="font-bold text-lg">Booking Template</h2>
        </div>
        ...
        <div className="flex gap-2 justify-end">
          <button
            onClick={() => setDraft(DEFAULT_BOOKING_TEMPLATE)}
